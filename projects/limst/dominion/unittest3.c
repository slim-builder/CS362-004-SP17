        ...
        for (deckCount = 0; deckCount <= 10; deckCount++)
        {
            for (discardCount = 0; discardCount <= 10; discardCount++)
            {
                ...
                if (discardCount == 0 && deckCount == 0)
                    ...
                    retValue = 0;
#if (NOISY_TEST == 1)
                printf("drawCard return value = %d, expected = %d\n", r, retValue);
#endif
                asserttrue(r == retValue, &globalFailFlag); // check if drawCard return value is correct when supply count of card is zero
                if (r == -1)
                    continue;
...
#if (NOISY_TEST == 1)
                printf("G.discardCount[p] = %d, expected = %d\n", G.discardCount[p], disValue);
#endif
                asserttrue(G.discardCount[p] == disValue, &globalFailFlag); // check if discard count is one
                if (deckCount != 0) {
...
#if (NOISY_TEST == 1)
                printf("G.deckCount[p] = %d, expected = %d\n", G.deckCount[p], deckValue);
#endif
                asserttrue(G.deckCount[p] == deckValue, &globalFailFlag); // check if discard count is one
                if (deckCount != 0) {
                ...
                if (deckCount != 0) {
#if (NOISY_TEST == 1)
                    printf("G.hand[p][handCount] = %d, expected = %d\n", G.hand[p][handCount], gold);
#endif
                    asserttrue(G.hand[p][handCount] == gold, &globalFailFlag); // check if discard count is one
                }
                else {
#if (NOISY_TEST == 1)
                    printf("G.hand[p][handCount] = %d, expected = %d\n", G.hand[p][handCount], silver);
#endif
                    asserttrue(G.hand[p][handCount] == silver, &globalFailFlag); // check if discard count is one
                }
